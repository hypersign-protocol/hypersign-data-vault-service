...
export class VaultRepository {
  ...
  async getVault(vaultFilterQuery: Partial<VaultsIndex>) {
    const vault = await this.vaultProvider.findOne(vaultFilterQuery);

    return vault;
  }

  async deleteVault(vaultFilterQuery: Partial<VaultsIndex>) {
    const vault = await this.vaultProvider.deleteOne(vaultFilterQuery);
